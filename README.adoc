...
multiple zones: `asia-east1-a`, `asia-east1-b`, `asia-east1-c`. 
...
gke-[cluster-name]-default-pool-be4471fe-qnl6   Ready    <none>   46s   v1.13.11-gke.23
----
...
The [hotspot file=0]`kubernetes.yaml` resource definition file is provided for you. If you are interested 
...
You can get the Project ID for your project by running `gcloud config get-value project` command.
...
for the `system` and `inventory` microservice.
...
| [hotspot=system-node-port file=0]`system.node.port`          | The HTTP port for the {kube} Service `system-service`, 9080 by default.
| [hotspot=inventory-node-port file=0]`inventory.node.port`     The HTTP port of the {kube} Service `inventory-service`, 9081 by default.
|===
...
[role='command']
```
gcloud container images delete gcr.io/[project-id]/system:1.0-SNAPSHOT
gcloud container images delete gcr.io/[project-id]/inventory:1.0-SNAPSHOT
```
